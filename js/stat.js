...
var renderCloud = function (ctx, props) {
  ...
  ctx.fillRect(props.getShadowPositionX(), props.getShadowPositionY(), props.width, props.height);
...
var renderTitle = function (ctx, props) {
  ...
  for (var i = 0, length = props.title.length; i < length; i += 1) {
    ...
    ctx.fillText(props.title[i], props.getTextBasePaddingX(), props.getTextBasePaddingY() + additionVerticalGap);
  }
};
...
var renderGraph = function (ctx, props, players, times) {
  ...
  for (var i = 0, length = players.length; i < length; i++) {
    ...
    var gistPositionY = graphPositionY + gistBaseHeight - gistHeight + lineHeight;

    // Y position for labels
    var timePositionY = gistPositionY - lineHeight / 2;
    var namePositionY = props.getTextBasePaddingY() + gistBaseHeight + titleHeight + lineHeight * 1.5;
